...
class Decoders {
    ...
    private static var __once: () = {
        ...
        // Decoder for [Error500]
        Decoders.addDecoder(clazz: [Error500].self) { (source: AnyObject, instance: AnyObject?) -> Decoded<[Error500]> in
            return Decoders.decode(clazz: [Error500].self, source: source)
        }

        // Decoder for Error500
        Decoders.addDecoder(clazz: Error500.self) { (source: AnyObject, instance: AnyObject?) -> Decoded<Error500> in
            if let sourceDictionary = source as? [AnyHashable: Any] {
                let _result = instance == nil ? Error500() : instance as! Error500
                ...
                return .failure(.typeMismatch(expected: "Error500", actual: "\(source)"))
            }
        }
        // Decoder for [HashMap]
        Decoders.addDecoder(clazz: [HashMap].self) { (source: AnyObject, instance: AnyObject?) -> Decoded<[HashMap]> in
            return Decoders.decode(clazz: [HashMap].self, source: source)
        }

        // Decoder for HashMap
        Decoders.addDecoder(clazz: HashMap.self) { (source: AnyObject, instance: AnyObject?) -> Decoded<HashMap> in
            if let source = source as? HashMap {
                return .success(source)
            } else {
                return .failure(.typeMismatch(expected: "Typealias HashMap", actual: "\(source)"))
            }
        }
        // Decoder for [Key]
